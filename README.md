...
- С++17 (C++1z)
## Планы по доработке
Расширить запрос, чтобы помимо стандартных свойств `id` и `type`, содержал ещё два:  
`from` — остановка, где нужно начать маршрут,  
`to` — остановка, где нужно закончить маршрут.  
Оба значения — названия существующих в базе остановок (они могут не принадлежат ни одному автобусному маршруту).  
На выход должны попасть имена остановок и автобусы, которые через них проходят.
***
